...
class Effect1DPlotter(object):
    ...
    def update(self, params):
        ...
        cs     = ax.contourf(D, W, P, p0r, cmap=cmap, norm=norm)
        ax.contour(cs, p0r, colors='0.1', linewidths=0.5)
        # plot contour lines:
        for pp in p0r[1:-1]:
        ...
        for pp,ll,cc in zip(p0rtx[::-1], labels, label_colors):
            ...
            if r is not None:
                ...
                ax.text(*r[i], ll, color=cc, ha='center', va='center', rotation=degrees(phi))
...
params  = Effect1DPlotParameters(n=40, d=1.0, w=20, dlim=(0.1,3.5), wlim=(3,60))
...
for n in range(3,41):
    ...
    fpath = os.path.join(dirREPO, 'content', 'img', f'contours_n={n}.png')
    plt.savefig( fpath )
...
# plt.savefig(  os.path.join(  os.path.dirname(__file__) , f'contours_n={n}.png'  )  )
